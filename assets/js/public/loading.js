...
(function setupFigmaAdminCardIcons() {
  ...
  style.id = 'figma-admin-card-icons-direct-style';
  style.textContent = `
  ...
  const icon = (name, className) => `<img class="${className}" src="${ICONS[name]}" alt="" aria-hidden="true">`;

  function countFromMeta(meta, symbol) {
    const node = [...(meta?.querySelectorAll('span') || [])].find(item => item.textContent.includes(symbol));
    const match = node?.textContent.match(/\d+/);
    return match ? match[0] : '0';
  }

  function decorateCard(card) {
    if (!card || card.dataset.figmaDirectIcons === '3') return;
    ...
    facts.innerHTML = `
      <span class="moderation-players">${icon('player', 'moderation-fact-icon')}<span>${playerText}</span></span>
      <span class="moderation-release">${icon('calendar', 'moderation-fact-icon')}<span>${releaseText}</span></span>
    ...
    card.dataset.figmaDirectIcons = '3';
  }
