(() => {
  const ICONS = {
    player: 'data:image/png;base64,',
    calendar: 'data:image/png;base64,',
    like: 'data:image/png;base64,',
    dislike: 'data:image/png;base64,'
  };

  function icon(name, className) {
    return `<img class="${className}" src="${ICONS[name]}" alt="" aria-hidden="true">`;
  }

  function decorateCard(card) {
    if (!card || card.dataset.figmaIconsReady === 'true') return;
    const players = card.querySelector('.moderation-players');
    const release = card.querySelector('.moderation-release');
    if (players) players.innerHTML = `${icon('player', 'moderation-fact-icon')}<span>${players.textContent.trim()}</span>`;
    if (release) release.innerHTML = `${icon('calendar', 'moderation-fact-icon')}<span>${release.textContent.trim()}</span>`;

    const meta = card.querySelector('.moderation-card-meta');
    if (meta) {
      const spans = [...meta.querySelectorAll('span')];
      const likeSpan = spans.find(span => span.textContent.includes('👍'));
      const dislikeSpan = spans.find(span => span.textContent.includes('👎'));
      const likeCount = likeSpan?.textContent.replace(/[^0-9]/g, '') || '0';
      const dislikeCount = dislikeSpan?.textContent.replace(/[^0-9]/g, '') || '0';
      const reactions = document.createElement('div');
      reactions.className = 'moderation-reactions-figma';
      reactions.innerHTML = `<span>${icon('like', 'moderation-reaction-icon')}<b>${likeCount}</b></span><span>${icon('dislike', 'moderation-reaction-icon')}<b>${dislikeCount}</b></span>`;
      card.querySelector('.moderation-card-side')?.appendChild(reactions);
      meta.classList.add('moderation-card-meta-clean');
    }
    card.dataset.figmaIconsReady = 'true';
  }

  function decorateAll(root = document) {
    root.querySelectorAll?.('.moderation-card').forEach(decorateCard);
  }

  decorateAll();
  const list = document.getElementById('suggestionModerationList');
  if (list) new MutationObserver(() => decorateAll(list)).observe(list, { childList: true, subtree: true });
})();
